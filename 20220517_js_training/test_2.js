...
console.log(dummyText.filter(text => text === 'of'));
console.log(dummyText.filter(text => text === 'of').length);

// Todo : Q-2. "dummy" 라는 문자열 원소를 모두 "sementic" 이라는 단어로 바꾸세요.

let dummy = dummyText.filter(text => text === 'dummy').map((item) => item = 'sementic');
console.log(dummy);
...
console.log(dummyText.reduce((acc, ele)=> acc + ele, []).length);
